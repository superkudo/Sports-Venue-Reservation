  ...
  methods: {
    ...
    initDate(left, right, d) {
      ...
      this.triggerEvent('mydata', {
        data: [parseInt(year ), parseInt(month) ,parseInt( day)]
      })
    },
    // 获取这周从周日到周六的日期
    calculateDate(_date) {
    ...
    dateSwiperChange(e) {
      ...
      this.setData({
        ...
        dateMonthValue:dDateFormat[1],
        dateYear: dDateFormat[0] + "年",
        dateYearValue: dDateFormat[0]
      })
    },
    // 获得日期字符串
    ...
    chooseDate(e) {
      ...
      let newm=moment(str,'YYYY-MM-DD')
      let Y=parseInt(newm.format('YYYY'));
      let M=parseInt(newm.format('MM'));
      let D=parseInt(newm.format('DD'));
    ...
    tiaomonth(e){
      ...
      let {dateYearValue,dateMonthValue,dateCurrent}=this.data
     
      let date=moment(dateYearValue+'-'+dateMonthValue+'-'+dateCurrent.getDate(),'YYYY-MM-DD')
      ...
      this.initDate(-5, 2, date); // 日历组件程序  -4左表示过去4周  右1表示过去一周
    }
  }
})